...
const PIPELINE: FlowStep[] = [
  ...
  },
  {
    ...
    title: "Deterministic checks flag, they never suspend",
    body: "Two pattern checks that cost nothing and run in milliseconds, both about where a page sends people. Naming a company is not one of them, and none of them can take a page down.",
    inputs: ["IP-logger blocklist", "Brand lookalike & punycode domains", "Big-company support claim"],
    branch: "Match → mandatory AI review on the stronger model, never served from cache, with the evidence attached: which check fired, the exact URLs, the brand, the decoded punycode.",
    tone: "auto",
  },
  {
    step: "4",
    label: "Automated · stage two",
    title: "AI review — the only thing that can suspend a page",
    body: "A multimodal call returns a structured verdict — approve or suspend, plus categories, a confidence score, a written reason, and exactly which signals fired. It suspends for two things: big-company support impersonation, and explicit content. Flagged pages arrive with the evidence and the brand's official domains, and the model has to confirm the flag rather than defer to it.",
    inputs: [
      "Profile text",
      "Every link URL",
      "Redirect target",
    ...
    branch: "Ambiguous, under the confidence threshold, or the provider errored → approved and logged for a human. Every suspension clears that threshold; there is no exemption for a pattern hit.",
    tone: "auto",
  },
];
...
const HUMAN_GATE: FlowStep[] = [
  {
    ...
    branch: "A suspension stays until a person reviews it. It is never lifted automatically, and never by the owner re-publishing.",
    tone: "human",
  },
  {
    step: "6",
