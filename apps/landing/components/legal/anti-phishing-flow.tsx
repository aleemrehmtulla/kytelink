...
const PIPELINE: FlowStep[] = [
  ...
  },
  {
    ...
    body: "Two pattern checks that cost nothing and run in milliseconds, both about where a page sends people. Naming a company is not one of them.",
    inputs: ["IP-logger blocklist", "Brand lookalike & punycode domains"],
    branch: "Match → suspended immediately at full confidence, no model call, signals recorded. A page claiming to be a big company's support desk is flagged here instead, and always sent to the AI.",
    tone: "auto",
  },
  {
    step: "4",
    ...
    body: "A multimodal call returns a structured verdict — approve or suspend, plus categories, a confidence score, a written reason, and exactly which signals fired. It suspends for two things: big-company support impersonation, and explicit content. Brand-flagged pages go to the stronger model with the brand's official domains to check against: links that stay on them approve.",
    inputs: ["Profile text", "Every link URL", "Redirect target", "Avatar image", "Official brand domains"],
    branch: "Ambiguous, under the confidence threshold, or the provider errored → approved and logged for a human. We under-block on purpose.",
    tone: "auto",
  },
];
